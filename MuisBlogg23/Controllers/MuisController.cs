...
namespace MuisBlogg23.Controllers
{
    ...
    public class MuisController : ControllerBase
    {
        ...
        public MuisController(IOptions<DatabasSettings> settings)
        {
            ...
            settings.Value.ConnectionString); var mongoDatabase = mongoClient.GetDatabase(
            settings.Value.DatabaseName); _muis = mongoDatabase.GetCollection<Muis>(
            settings.Value.MuisCollectionName);
        }
        [HttpGet]
